...
if [ -n "$API_KEY" ]; then
  ...
  curl -s -b /tmp/cookies.txt -H "Origin: http://localhost:5173" \
    http://localhost:3000/api/admin/api-keys | jq '.data[] | {id, name, enabled, requestCount}'
  echo
  ...
  echo "$SERVERS_RESPONSE" | jq -c '. | if type == "object" and has("data") then "✅ API key works! Response: \(.)" elif type == "array" then "✅ API key works! Found \(length) servers" else . end'
  echo
  
  if [ -n "$API_KEY_ID" ]; then
    echo "5. Getting API key usage stats..."
    curl -s -b /tmp/cookies.txt -H "Origin: http://localhost:5173" \
      "http://localhost:3000/api/admin/api-keys/$API_KEY_ID/usage" | jq '.'
    echo
    
    echo "6. Updating API key name..."
    curl -s -b /tmp/cookies.txt -H "Origin: http://localhost:5173" \
      -X PATCH "http://localhost:3000/api/admin/api-keys/$API_KEY_ID" \
      -H "Content-Type: application/json" \
      -d '{"name": "Updated Test Key"}' | jq '.'
    echo
    
    echo "7. Revoking API key..."
    curl -s -b /tmp/cookies.txt -H "Origin: http://localhost:5173" \
      -X DELETE "http://localhost:3000/api/admin/api-keys/$API_KEY_ID" | jq '.'
    echo
    
    echo "8. Verifying revoked key doesn't work..."
